...
def build_optimizer(config: dict, parameters) -> Optimizer:
    ...
    base_lr = config.get("learning_rate", 3.0e-4)
    ...
    if scheduler_name == "plateau":
        ...
        return (
            lr_scheduler.OneCycleLR(
                ...
                total_steps=config.get("epochs", 60),
        ...
        warmup = config.get("learning_rate_warmup", 4000)
        peak_rate = config.get("learning_rate", 1.0e-4)
        decay_length = config.get("learning_rate_decay_length", 1000)
        return (
