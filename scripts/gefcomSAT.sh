...
for seq_len in 744 336 168 96; do
  for pred_len in 744 336 168 96; do
    for zone in CT MASS ME NEMASSBOST NH RI SEMASS TOTAL VT WCMASS; do
      for attention_window in 24; do
        python train.py --train \
          ...
          --amp --zone=${zone} \
          --exp_out=gef/TransformerSATConv/${zone} --model=TransformerSAT \
