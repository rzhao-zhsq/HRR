...
for pred_len in 24 168 744; do
  for zone in CT MASS ME NEMASSBOST NH RI SEMASS TOTAL VT WCMASS; do
    for d_layers in 6; do
      for kernel in 3 5 7 9 11 13 15 17 19; do
        for beta in 1e-4 1e-3; do
          python train.py --train \
            ...
            --zone=${zone} --exp_out=gefcom2017/VConvSingle/${zone} --model=VConv \
            --combine_type=latent_residual --beta=${beta} --sample_size=1 --valid_temperature=0.0 &
        done
        wait
      done
    done
