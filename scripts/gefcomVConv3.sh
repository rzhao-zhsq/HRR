      ...
      for combine_type in latent_only latent_residual; do
        for beta in 1e-4 1e-3; do
          python train.py --train \
            ...
            --zone=${zone} --exp_out=gefcom2017/VConv2Stage/${zone} --model=VConv \
            --conv_gaussian \
            --load_ckpt
            --pretrained_model=gef/Conv/${zone}/gefcom_reg_Conv_s-0_l-0_p-${pred_len}_d-64_el-6_norm-none_embed-learned-f-h_ec-2_dc-2_oc-1_drop-0.3-residual/checkpoint.pth \
            --combine_type=${combine_type} --beta=${beta} --sample_size=1000 --valid_temperature=1.0 &
        done
      done
      wait
    done
