...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # )
        # if hasattr(configs, "pretrained_path") and configs.pretrained_path is not None:
    ...
    def forward(
        ...
        enc_out_dict = self.encoder(inputs_embeds=enc_embed, return_dict=True)
        output = self.pred_head(enc_out_dict['last_hidden_state'][:, -self.pred_len:, :])
        ...
        return {
            "output": output,
            # encoder output
            "encoder_output": enc_out_dict['last_hidden_state'],
            "encoder_attns": enc_out_dict.attentions,
        }
