...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        return {
            "output": V.contiguous(),
            "attn": A if self.output_attention else None
        }
...
class ProbAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        return {
            "output": context.contiguous(),
            "attn": attn
        }



class ResembleFullAttention(nn.Module):
    """
    ...
    def __init__(
            ...
            sub_space=False, sub_space_residual=False, out_space=False, out_space_residual=False,
            diversity_metric="bvc",
    ):
        super(ResembleFullAttention, self).__init__()
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        if self.sub_space_residual or self.sub_space:
            sub_d = self.get_disgreement(values)
        ...
        return {
            "output": V,
            "attn": A if self.output_attention else None,
            "sub_disagreement": sub_d if self.sub_space else None,
            "out_disagreement": out_d if self.out_space else None,
        }
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        attn_dict = self.inner_attention(
            queries, keys, values, attn_mask
        )
        out = attn_dict.get("output", None)
        attn_dict['output'] = self.out_projection(out.view(B, L, -1))
        return attn_dict


# class ResembleAttentionLayer(nn.Module):
#     def __init__(self, attention, d_model, n_heads, d_keys=None, d_values=None):
#         super(ResembleAttentionLayer, self).__init__()
#
#         d_keys = d_keys or (d_model // n_heads)
#         d_values = d_values or (d_model // n_heads)
#
