...
class PatchTST_backbone(nn.Module):
    def __init__(
            self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # model
        z = self.backbone.forward(z)  # z: [bs x nvars x d_model x patch_num]
        z = self.head.forward(z)  # z: [bs x nvars x target_window]
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    ...
    def forward(self, x) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]
        ...
        d_model = self.W_P.out_features

        # Input encoding
        x = x.permute(0, 1, 3, 2)  # [bs x nvars x patch_num x patch_len]
        x = self.W_P(x)  # [bs x nvars x patch_num x d_model]

        x = x.reshape(-1, patch_num, d_model)  # [bs * nvars x patch_num x d_model]
        x = self.input_dropout(x + self.W_pos)  # [bs * nvars x patch_num x d_model]

        # Encoder
        x = self.encoder(x)  # [bs * nvars x patch_num x d_model]
        x = x.reshape(-1, n_vars, patch_num, d_model)  # [bs x nvars x patch_num x d_model]
...
class TSTEncoder(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                TSTEncoderLayer(
                    q_len, d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights = self.dropout(
            torch.softmax(attn_scores, dim=-1))  # attn_weights   : [bs x n_heads x max_q_len x q_len]
